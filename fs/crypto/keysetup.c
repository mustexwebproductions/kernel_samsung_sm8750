...
#ifdef CONFIG_DDAR
	kmem_cache_free(fscrypt_inode_info_cachep, ext_ci);
#else
...
{
#ifdef CONFIG_DDAR
	struct ext_fscrypt_inode_info *ext_crypt_info;
#endif
...
#ifdef CONFIG_DDAR
	ext_crypt_info = kmem_cache_zalloc(fscrypt_inode_info_cachep, GFP_KERNEL);
	if (!ext_crypt_info)
		return -ENOMEM;
...
int fscrypt_get_encryption_info(struct inode *inode, bool allow_unsupported)
{
	...
	if (fscrypt_has_encryption_key(inode)) {
		...
		if (fscrypt_dd_encrypted_inode(inode) && fscrypt_dd_is_locked()) {
			dd_error("%s - failed to open in lock state (ino:%ld)\n",
		...
		if (ext_ci->ci_dd_info) {
			if (fscrypt_dd_is_locked()
					&& !fscrypt_dd_flg_gid_restricted(
						(ext_ci->ci_dd_info->policy.flags
...
int fscrypt_set_ddar_context(struct inode *inode, struct fscrypt_inode_info *ci,
		union fscrypt_context *ctx, int ctxsize, void *fs_data)
{
	if (fscrypt_has_dar_info(inode)) {
		...
		if (ext_ci->ci_dd_info) {
			...
			res = dd_write_crypt_context(
					inode, &ext_ci->ci_dd_info->crypt_context, fs_data);
			dd_verbose("%s - ino : %ld, policy.flag:%x, res : %d", __func__,
					inode->i_ino, ext_ci->ci_dd_info->policy.flags, res);
		}
		return res;
	}
	return -EAGAIN;
